...
export default function DashboardEmulator({ onClose }: { onClose: () => void }) {
  const { lang } = useApp();
...
• Inventory & Stock Audit: Update stock levels and scan barcodes using the mobile phone camera.`
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-[#080b13] font-sans text-slate-100 select-none" dir={isRtl ? "rtl" : "ltr"}>
      <AnimatePresence>
        {sidebarOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            ...
            className="absolute inset-0 z-40 bg-black/60 backdrop-blur-[2px] md:hidden"
          />
        )}
      </AnimatePresence>
      ...
      <div
        className={cn(
          "z-50 flex h-full w-[min(280px,88vw)] shrink-0 flex-col justify-between border-e border-white/5 bg-[#0b0f19]",
          "fixed inset-y-0 start-0 transition-transform duration-300 ease-out md:relative md:w-56 md:translate-x-0 lg:w-60",
          sidebarOpen
            ? "translate-x-0"
            : isRtl
      ...
      >
        <div>
          ...
          <div className="flex items-center gap-3 border-b border-white/5 p-4">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/5 bg-[#141a2e] shadow-[0_0_10px_rgba(255,255,255,0.08)]">
              <img
                src="/5.png"
                alt="Kodify Logo"
            ...
            <div className={cn("flex min-w-0 flex-col", isRtl ? "text-right" : "text-left")}>
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">{lang === "ar" ? "مرحباً" : lang === "ku" ? "بەخێربێیت" : "WELCOME"}</span>
              <span className="text-xs font-black text-white">{lang === "ar" ? "الكاشير" : lang === "ku" ? "کاشێر" : "Cashier"}</span>
            </div>
            <span className="ms-auto shrink-0 rounded-full border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-400">
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-extrabold transition-all",
                    isRtl ? "text-right" : "text-left",
                    isActive
                      ? "border border-indigo-500/30 bg-indigo-500/15 text-white shadow-[0_0_15px_rgba(99,102,241,0.15)]"
                ...
                >
                  ...
                  {isActive && <div className="h-3 w-1 shrink-0 rounded-full bg-indigo-500" />}
                </button>
              );
            })}
          </div>
        ...
        <div className="border-t border-white/5 p-3">
          <button
            onClick={onClose}
            className={cn(
              "flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-xs font-extrabold text-rose-500 transition-all hover:bg-rose-500/10 hover:text-white",
              isRtl ? "text-right" : "text-left"
            )}
          >
            <LogOut size={16} />
      ...
      {/* 2. MAIN CONTENT AREA */}
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden bg-[#080b13]">
        {/* Mobile top bar */}
        <div className="flex shrink-0 items-center gap-2 border-b border-white/5 bg-[#0b0f19]/95 px-3 py-2.5 md:hidden">
          <button
            onClick={() => setSidebarOpen(true)}
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/5 bg-[#141a2e] text-slate-300 transition-colors hover:text-white"
          ...
          {activeTab === "pos" && (
            <button
              ...
              className="relative flex shrink-0 items-center gap-1.5 rounded-xl border border-indigo-400/20 bg-indigo-500/10 px-3 py-2 text-[10px] font-bold text-indigo-300"
            >
              <ShoppingBag size={14} />
              <span>{currT.cart}</span>
              {totalItemCount > 0 && (
                <span className="absolute -top-1.5 -end-1.5 flex h-4 w-4 items-center justify-center rounded-full border border-[#080b13] bg-pink-500 text-[8px] font-black text-white">
                  {totalItemCount}
                </span>
              )}
            </button>
        ...
        {activeTab === "pos" && (
          <div className="flex min-h-0 flex-1 flex-col overflow-hidden p-3 sm:p-4 md:p-6">
            ...
            <div className="mb-3 flex shrink-0 flex-col gap-2 sm:mb-4 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
              <div className="relative w-full min-w-0 flex-1 sm:max-w-lg">
              ...
              <div className="flex w-full shrink-0 items-center gap-2 sm:w-auto">
                <button
                  ...
                  className="rounded-xl border border-white/5 bg-[#141a2e] p-2.5 text-slate-400 transition-all hover:bg-slate-800 hover:text-white active:scale-[0.98]"
                ...
                <button
                  onClick={() => setIsCartOpen(true)}
                  className="relative flex flex-1 items-center justify-center gap-2 rounded-xl border border-indigo-400/20 bg-indigo-500/10 px-4 py-2.5 text-xs font-bold text-indigo-300 shadow-[0_0_15px_rgba(99,102,241,0.1)] transition-all hover:bg-indigo-500/25 sm:flex-none sm:shrink-0"
                >
                  <ShoppingBag size={16} />
                  <span>{currT.cart}</span>
                  {totalItemCount > 0 && (
            ...
            <div data-lenis-prevent className="flex-1 overflow-y-auto pe-1 pb-4 min-h-0 pos-scrollbar">
              {filteredProducts.length === 0 ? (
                ...
                <div className="grid grid-cols-1 min-[420px]:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2.5 sm:gap-3">
        ...
        {activeTab === "inventory" && (
          <div data-lenis-prevent className="flex-1 space-y-4 overflow-y-auto p-3 pos-scrollbar sm:space-y-6 sm:p-6">
            {/* Header */}
              ...
              <div className="lg:col-span-2 bg-[#0f1423] border border-white/5 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-7 gap-1 sm:gap-2">
                  {daysGrid.map((day, idx) => {
                    if (day === null) {
                      return <div key={`empty-${idx}`} className="h-12 rounded-xl border border-transparent bg-[#141b2f]/30 sm:h-16" />;
                    }
                    ...
                    return (
                      <button
                        ...
                        className={`flex h-12 flex-col justify-between rounded-xl border p-1.5 transition-all sm:h-16 sm:p-2 ${
        ...
        {isCartOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute end-0 top-0 bottom-0 z-40 flex w-full max-w-[380px] flex-col justify-between border-s border-white/5 bg-[#0e1322] p-4"
            >
              <div>
                {/* Header */}
